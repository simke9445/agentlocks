...
**The one gap, stated up front.** The gate runs on the publish runner (`linux-x64`), so it executes only one of four binaries. A corrupt `darwin-arm64`, `darwin-x64`, or `linux-arm64` binary passes the gate and is `latest` for the flip-to-canary window, caught by the post-flip canary (a bad *manifest* on any platform is caught earlier, by the pre-flip manifest guard, Section 4.4/6). That window is a real control only because the canary's **notify-on-failure is mandatory** and the roll-back is owned: the release maintainer (the approver who clicked the environment gate) begins the Section 5 roll-back within 30 minutes of the page. A watch-the-canary-manually arrangement bounds the window by when someone happens to look, i.e. not at all; adopt it only by also accepting an unbounded window and dropping the single-digit-minutes claim. Closing the gap pre-flip means gating the four-target matrix on the flip, which the tokenless design defers to npm Staged Publishing (Section 4.3).

**Supply-chain pinning is in-scope, not deferred.** The release job holds `id-token: write` and publishes to `latest`, so it cannot rest on mutable inputs: a compromised floating action or an `npm@latest` update landing *after* the human approval could publish bad bytes, bypassing every in-repo gate. This change therefore SHA-pins the three third-party actions (`actions/checkout`, `oven-sh/setup-bun`, `actions/setup-node`) and replaces `npm install -g npm@latest` with a vetted npm pin (Section 6), so the guarantee does not rest on anything mutable.

One operating rule falls out of all this: **never re-cut a version.** A pre-flip failure is fixed by re-running, *unless* it is a defect in already-published immutable bytes (a bad platform binary, or a pre-existing `main@VERSION`), which is patch-forward; a bug found after the flip is a `latest` roll-back plus `npm deprecate` plus patch-forward (Section 5/6).
...
release  (environment: release, OIDC, ONE human approval) ─────► canary
   ...
   publish 4 platform packages -> latest (skip-if-exists)            first step self-checks
   pack main + inject & assert exact deps                              npm latest == VERSION,
   ...
   emit flipped=true output  (fast in-run signal)                    mandatory notify)
...
**Gating the canary correctly is subtle and load-bearing.** The post-flip steps can fail on benign CDN lag (or a hard `got_wrong` terminal check reds the job), so the canary must not key on the whole `release` job succeeding. Run it `if: ${{ always() }}` (a plain `if:` is implicitly ANDed with `success()`, which a post-flip job failure would make false, skipping the canary exactly when the three non-linux binaries are live and unproven). Its FIRST step decides whether a flip to `$VERSION` happened, robustly against both a lost output and propagation lag: it proceeds the moment the `flipped` hint is `'true'` **or** a bounded poll of `npm view agentlocks@latest version --prefer-online` resolves to `$VERSION` (the poll spans documented `latest` propagation, so a lagged-but-real flip is never falsely skipped). If the budget elapses with neither, it disambiguates by `needs.release.result`: `success` means the job ran the flip to completion, so a `latest` still not at `$VERSION` is a real anomaly and the canary fails and notifies; otherwise it was a pre-flip abort (no flip happened) and the canary exits 0 with a `::warning::`, not a false alarm. Keying on the registry, not a job output alone, is what makes this robust to a runner dying between the flip's `npm publish` and recording the output: the `flipped` output (`id: flip`, declared in the `release` job's `outputs:` map) is only a fast hint, the npm poll is the authority.
...
for i in $(seq 1 24); do
  ...
  if npm_config_omit= npm install -g --prefix "$GATE" --prefer-online --include=optional "./$TARBALL"; then
    ...
    if GOT="$(AGENTLOCKS_DISABLE_BUN_FALLBACK=1 "$GATE/bin/agentlocks" --version)"; then
      # primary check is a REAL lock cycle: a bun-compiled binary's failure mode is asset extraction
      # on the first real command, which --version/--help bypass.
      WT="$(mktemp -d)"
      if ( cd "$WT" \
...
The gate runs on `linux-x64` only, so **nothing it proves covers the execution of the three non-linux binaries**. A corrupt one passes the gate (a bad `os`/`cpu`/`libc`/`name`/`files` manifest is instead caught pre-flip by the manifest guard below); the launcher treats a skipped or absent optionalDependency as non-fatal (`bin/agentlocks.mjs:79-92`), so that platform silently drops out of a default install and reaches users for the flip-to-canary window. Per-package CDN skew compounds it: `agentlocks@V` can be installable while a platform `@V` isn't yet, so npm skips the optional dep and installs a wrapper with no binary even when every binary is fine. The flip moves `latest` before any of this is observable on the three targets; the canary is the only net.

Reframe the existing matrix `verify` job as the canary:

- **Post-flip alarm, not a gate**, `needs: release` + `if: ${{ always() }}`. Its first step proceeds with the matrix when the `flipped` hint is `'true'` **or** a bounded `npm view agentlocks@latest version --prefer-online` poll resolves to `$VERSION` (the poll covers propagation lag, so a lagged real flip is not falsely skipped); if the budget elapses with neither, it fails and notifies when `needs.release.result == 'success'` (the flip ran but `latest` is wrong, a real anomaly) and otherwise exits 0 with a `::warning::` (a pre-flip abort, no flip). Keying on the registry, not the `flipped` output alone (Section 4.2), keeps a runner dying between the flip and the output write from orphaning the only post-flip alarm.
- **All four targets**, `fail-fast: false`: `ubuntu-latest`, `ubuntu-24.04-arm`, `macos-latest`, and `macos-15-intel` for Intel macOS. `macos-15-intel` and `macos-26-intel` are currently supported Intel labels; hosted Intel-macOS is being wound down, so **re-check GitHub's runner-images list for a supported x86_64 label before implementing**. When hosted x86_64 ends, choose a self-hosted Intel runner, drop darwin-x64, or scope it fallback-only (the Section 4.1 honesty).
- **Each leg tests BOTH unpinned `agentlocks` AND exact `agentlocks@$VERSION`** in separate clean prefixes, Bun fallback off, each with `npm_config_omit= --include=optional` like the gate (so a runner `.npmrc` carrying `omit=optional` cannot skip the platform package and false-fail the canary into a needless rollback). The unpinned install is the real user contract on these edges and the only thing that catches the CDN-skew skipped-optionalDependency case; the exact pin proves the binary. Each runs the **same real lock cycle as the gate** (`acquire f.txt --reason ci-canary --id-only` then `release`), not `--version`/`--help`.
- **`permissions: {}`** (no `contents`, no `id-token`): it neither writes the repo nor publishes, and running a freshly-published binary with `id-token: write` is gratuitous OIDC exposure. The in-job gate and terminal check (Section 4.3, 4.5) execute the same registry bytes inside the `id-token: write` release job (where the flip needs that token). They run first-party just-built bytes, but as defense-in-depth empty the publish credentials (`NODE_AUTH_TOKEN`, `GITHUB_TOKEN`, `ACTIONS_ID_TOKEN_REQUEST_TOKEN` / `_URL`) via `env:` on those install/exec steps, so the executed binary cannot reach the OIDC token.
...
Cheap pre-flip hardening that doesn't execute the three binaries: the **published-manifest guard** (Section 6 step 9) asserts each platform package's `name`/`version`/`os`/`cpu`/`libc`/`files` against what main pins, plus that the published tarball contains the `bin/agentlocks` file the launcher resolves by path (the platform packages declare no package.json `bin` field), and waits for all four to be installable, before flipping. It catches a bad manifest or an unpropagated package, but not a corrupt binary; only a matrix-gated flip closes that.
...
concurrency:
  ...
  queue: max              # FIFO, up to 100 pending; do not silently drop a queued tag (needs cancel-in-progress: false)
  cancel-in-progress: false   # a cancelled mid-flip is the half-done state we avoid
```

State its limits so it isn't over-trusted:

- **Set `queue: max` so a queued run is not silently dropped.** The default `queue: single` keeps at most one pending run and cancels an older pending one when a newer queues, so `v0.6.3` could be discarded if `v0.6.4` is pushed while it waits. GitHub added `queue: max` (changelog 2026-05-07): up to 100 pending runs, processed in the order they started waiting, incompatible with `cancel-in-progress: true` (fine here, we use `false`). It is FIFO by wait-start, not by *version*, so an out-of-order tag push can still run older-after-newer; keep the monotonic guard below for that, and the Section 6 step 10 procedure for the manual-terminal gap `queue:` cannot reach.
...
| In-job gate fails before the flip (build flake, lag, wiring) | Nothing. main was only packed, never published. | Re-run for a no-code-change cause (lag, flake). Any source change is patch-forward once immutable platform bytes exist for this version (a re-run can replace only the still-unpublished main); don't re-cut. |
...
1. **Top-level `concurrency`** block, constant group, `queue: max`, `cancel-in-progress: false` (Section 4.6).
...
7. **Move Create-GitHub-Release (lines 107-123) to after the flip but before the terminal check, and make it idempotent.** Binaries are already on disk (same job), so no artifact plumbing. After the flip so a Release never describes a non-`latest` version; before the terminal check so benign lag reddening that check can't skip the Release. Make it recover on re-run (`gh release view "$GITHUB_REF_NAME" >/dev/null 2>&1 || gh release create ...`, uploading the on-disk binaries): the flip's pre-existing-`main` hard-fail (step 6) aborts a re-run before this step, so without idempotent recovery a flipped version whose Release step failed could never get its Release. Don't split into a rebuilding job (`bun build --compile` isn't byte-reproducible).
8. **Reframe `verify` as `canary`** (Section 4.4): `needs: release` + `if: ${{ always() }}`, first step proceeds when `flipped == 'true'` OR a bounded `npm view agentlocks@latest == $VERSION` poll succeeds (covers propagation lag), fails-and-notifies if the budget elapses while `needs.release.result == 'success'`, else exits 0 with a warning (no flip); keying on the registry not the output alone, so a lost output or dead runner cannot orphan the alarm; four targets (`macos-15-intel`/`macos-26-intel` for Intel, a currently supported x86_64 label, re-check per Section 4.4); both unpinned and exact installs (with `npm_config_omit= --include=optional` as the gate, so an inherited `omit=optional` cannot false-fail the canary), fallback off, real lock cycle; `permissions: {}`; a mandatory notify-on-failure routed around `permissions: {}` (webhook, or a separate `notify` job with only `issues: write`).
9. **Pre-flip published-manifest guard** over all four packages, **run before the gate and flip (steps 5-6) despite its number here** (a superset of the linux-`libc`-only check): validate `name`/`version`/`os`/`cpu`/`libc`/`files` via `npm view agentlocks-<d>@$VERSION --json` against what main pins, plus that the published tarball contains the `bin/agentlocks` file the launcher resolves by path (the platform packages declare no package.json `bin` field, so do not assert one), and wait for all four to be installable, before the flip.
10. **Rewrite `RELEASING.md` to this model** (same PR): replace "the `verify` job is the real gate" (the gate is now the in-job pre-flip check); recast "Cutting a release" to stage -> gate -> flip -> canary; **replace** the delegation prompt's "if a step fails after publishing, re-run (publishes are idempotent)" (line 69) with the split: a gate failure re-runs ONLY for a no-code-change cause (lag/flake) and is patch-forward for any source change once immutable platform bytes exist for the version (a re-run replaces only unpublished main); a canary failure is roll-back; a prior-published-binary defect is patch-forward (the current "publishes are idempotent" line instructs the exact wrong recovery for the defective-binary and source-change cases); qualify "nothing to rotate" (recovery is a 2FA action); add the serialization rule (no manual `dist-tag`/publish, and no new release tag, while a run is queued/running; name the maintainer as roll-back owner with the 30-min commitment).

**Pin npm and the actions in this PR.** Replace `npm install -g npm@latest` (`release.yml:42`) with a pin to a vetted npm at/above the OIDC-publish floor (>= 11.5.1) and older than the project's package-age window, so an npm release landing after the human approval can't publish bad bytes. **SHA-pin the three third-party actions** (`actions/checkout`, `oven-sh/setup-bun`, `actions/setup-node`) to commit SHAs in the same change, not a follow-up: a job holding `id-token: write` that publishes to `latest` cannot rest on mutable action tags, since a compromised tag update landing after the human approval would bypass every in-repo gate. Deferring it would leave the headline safety property only conditionally met, so it is in-scope here.
