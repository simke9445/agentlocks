...
- **Post-flip alarm, not a gate**, gated on `needs: release` + `if: ${{ always() && needs.release.outputs.flipped == 'true' }}` (the `always()` and the `id: flip`/`outputs:` wiring from Section 4.2 are mandatory or it never runs / gets skipped on a post-flip job failure). Backstop the trigger against a runner dying between the flip's `npm publish` returning and the `flipped` output being recorded (which would orphan the only post-flip alarm): the canary runs and self-checks `npm view agentlocks@latest version`, proceeding if it equals `$VERSION` even when `flipped` is empty, so a lost output cannot silence it.
...
8. **Reframe `verify` as `canary`** (Section 4.4): `needs: release` + `if: ${{ always() && needs.release.outputs.flipped == 'true' }}`; four targets (`macos-15-intel`/`macos-26-intel` for Intel, a currently supported x86_64 label, re-check per Section 4.4); both unpinned and exact installs, fallback off, real lock cycle; `permissions: {}`; a mandatory notify-on-failure routed around `permissions: {}` (webhook, or a separate `notify` job with only `issues: write`).
