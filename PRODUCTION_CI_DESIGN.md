...
8. **Reframe `verify` as `canary`** (Section 4.4): `needs: release` + `if: ${{ always() }}`, first step proceeds when `flipped == 'true'` OR a bounded `npm view agentlocks@latest == $VERSION` poll succeeds (covers propagation lag), fails-and-notifies if the budget elapses while `needs.release.result == 'success'`, else exits 0 with a warning (no flip) — keying on the registry not the output alone, so a lost output or dead runner cannot orphan the alarm; four targets (`macos-15-intel`/`macos-26-intel` for Intel, a currently supported x86_64 label, re-check per Section 4.4); both unpinned and exact installs, fallback off, real lock cycle; `permissions: {}`; a mandatory notify-on-failure routed around `permissions: {}` (webhook, or a separate `notify` job with only `issues: write`).
