...
- **All four targets**, `fail-fast: false`: `ubuntu-latest`, `ubuntu-24.04-arm`, `macos-latest`, and `macos-15-intel` for Intel macOS. `macos-15-intel` is the current supported Intel label and the **last x86_64 Actions image**, available until roughly August 2027, after which hosted x86_64 is unsupported (`macos-26-intel` also runs Intel; the older `macos-13` image is on its deprecation path). Re-check the label before that sunset; when hosted x86_64 ends, choose a self-hosted Intel runner, drop darwin-x64, or scope it fallback-only (the Section 4.1 honesty).
...
8. **Reframe `verify` as `canary`** (Section 4.4): `needs: release` + `if: ${{ always() && needs.release.outputs.flipped == 'true' }}`; four targets (`macos-15-intel` for Intel, the last x86_64 Actions image per Section 4.4); both unpinned and exact installs, fallback off, real lock cycle; `permissions: {}`; a mandatory notify-on-failure routed around `permissions: {}` (webhook, or a separate `notify` job with only `issues: write`).
