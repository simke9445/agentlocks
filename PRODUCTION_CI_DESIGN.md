...
**Gating the canary correctly is subtle and load-bearing.** The post-flip steps can fail on benign CDN lag (or a hard `got_wrong` terminal check reds the job), so the canary must not key on the whole `release` job succeeding. Run it `if: ${{ always() }}` (a plain `if:` is implicitly ANDed with `success()`, which a post-flip job failure would make false, skipping the canary exactly when the three non-linux binaries are live and unproven), and make its FIRST step self-determine whether the flip happened by querying the registry: `npm view agentlocks@latest version --prefer-online` equal to `$VERSION` means the flip landed (proceed with the matrix), anything else means it did not (exit 0, nothing to canary). Keying on the registry, not a job output, is what makes this robust: even if the runner died between the flip's `npm publish` returning and recording any output, the `always()` canary still runs and its self-check sees the moved `latest`. The flip step still emits a `flipped=true` output (`id: flip`, declared in the `release` job's `outputs:` map) as a fast in-run signal, but the npm self-check is the authority, so a lost output is harmless.
...
- **Post-flip alarm, not a gate**, `needs: release` + `if: ${{ always() }}`. Its first step self-checks `npm view agentlocks@latest version --prefer-online` and proceeds with the matrix only if that equals `$VERSION` (the flip landed), else exits 0. Keying on the registry rather than the `flipped` job output (Section 4.2) makes the trigger robust to a runner dying between the flip and the output write: the `always()` job still runs and the npm self-check still sees the moved `latest`, so the only post-flip alarm cannot be orphaned.
...
8. **Reframe `verify` as `canary`** (Section 4.4): `needs: release` + `if: ${{ always() }}`, with a first step that self-checks `npm view agentlocks@latest version == $VERSION` and exits 0 if the flip did not land (keying on the registry, not the `flipped` output, so a lost output or a dead runner cannot orphan the alarm); four targets (`macos-15-intel`/`macos-26-intel` for Intel, a currently supported x86_64 label, re-check per Section 4.4); both unpinned and exact installs, fallback off, real lock cycle; `permissions: {}`; a mandatory notify-on-failure routed around `permissions: {}` (webhook, or a separate `notify` job with only `issues: write`).
