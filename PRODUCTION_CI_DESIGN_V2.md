...
| linux-x64-musl | `ubuntu-latest` + `container: node:22-alpine` |
| linux-arm64-musl | `ubuntu-24.04-arm` + `container: node:22-alpine` |
...
2. **`concurrency`** block: constant group, `cancel-in-progress: false` (no `queue:` key exists; one-tag-at-a-time is a manual invariant, see Section 4.6).
...
8. **Pin every mutable input in this PR**, so the flip job's `id-token: write` rests on nothing mutable: SHA-pin the third-party actions (`actions/checkout`, `oven-sh/setup-bun`, `actions/setup-node` to commit SHAs); replace `npm install -g npm@latest` with a vetted npm pin (at or above the OIDC floor, inside the package-age window); pin Bun and Node via the `setup-bun` `bun-version` and `setup-node` `node-version` inputs (they produce and run the release bytes); and pin the `node:22-alpine` musl-verify container by digest (`node:22-alpine@sha256:...`), not a floating tag, since it verifies release bytes. Record each pinned version's release date as package-age evidence. (Housekeeping, not gating: prune stale `preflight` tags from abandoned releases periodically; they are harmless because main pins platform packages by exact version, never by their tag.)
