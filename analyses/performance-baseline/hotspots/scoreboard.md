...
| --- | ---: | ---: | ---: | ---: | --- |
| Build-time injected package `name`/`version` constants replacing all `package.json` imports | 1 | 4 | 2 | 2.0 | Implemented in `2d67e34`; clean result: -1,277 raw bytes, -557 gzip bytes, -567 packed bytes. |
...
| Large-N active-lock read/parse/sort attribution for `git_begin`/acquire/status/refresh | 4 | 3 | 4 | 3.0 | Implemented in `829b833`; clean targeted p95 paired-delta wins at 1,000 locks: acquire -91.15 ms, refresh -62.61 ms, status -54.37 ms, git begin -124.20 ms. |
