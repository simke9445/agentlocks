...
| --- | --- | --- | --- | --- |
| `commander` | `14.0.3` | runtime | `2026-01-31T01:47:17.592Z` | 123 days |
...
- `commander` remains the only declared runtime dependency and stays pinned at `14.0.3`. The
  published CLI ships as one bundled `dist/agentlocks.mjs`, but the manifest intentionally keeps the
  source dependency explicit for the current release line. Re-evaluate before the next npm release if
  the goal becomes a dependency-free install surface.
