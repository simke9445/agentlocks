...
test("F1 re-acquiring your own path is idempotent (same id, exit 0)", async () => {
  await withWorkspace(async (dir) => {
    ...
    const first = await reg.acquire({ resourceSpecs: ["a.ts"], reason: "first" });
    const second = await reg.acquire({ resourceSpecs: ["a.ts"], reason: "retry" });
...
test("F1 a broader request over your own narrow lock still conflicts (exit 3)", async () => {
  await withWorkspace(async (dir) => {
    const reg = registry(dir, AGENT_A);
    await reg.acquire({ resourceSpecs: ["src/a.ts"], reason: "narrow" });
    const broad = await reg.acquire({ resourceSpecs: ["src/**"], reason: "broad" });
...
test("F1 a held glob covers a re-acquired concrete path (idempotent)", async () => {
  await withWorkspace(async (dir) => {
    const reg = registry(dir, AGENT_A);
    const glob = await reg.acquire({ resourceSpecs: ["src/**"], reason: "glob" });
    const concrete = await reg.acquire({ resourceSpecs: ["src/a.ts"], reason: "concrete" });
...
test("F1 another agent's overlapping lock still conflicts (not idempotent)", async () => {
  await withWorkspace(async (dir) => {
    await registry(dir, AGENT_A).acquire({ resourceSpecs: ["a.ts"], reason: "A" });
    const b = await registry(dir, AGENT_B).acquire({ resourceSpecs: ["a.ts"], reason: "B" });
...
test("F2 status --mine shows only the caller's locks", async () => {
  await withWorkspace(async (dir) => {
    await registry(dir, AGENT_A).acquire({ resourceSpecs: ["a.ts"], reason: "A" });
    await registry(dir, AGENT_B).acquire({ resourceSpecs: ["b.ts"], reason: "B" });
...
test("F2 release --mine drops every lock the caller holds; no-op when none", async () => {
  await withWorkspace(async (dir) => {
    const reg = registry(dir, AGENT_A);
    await reg.acquire({ resourceSpecs: ["a.ts"], reason: "A" });
    await reg.acquire({ resourceSpecs: ["b.ts"], reason: "A2" });
    await registry(dir, AGENT_B).acquire({ resourceSpecs: ["c.ts"], reason: "B" });
...
test("F3 a held lock covers the staged path, annotated owned_by_caller", async () => {
  await withWorkspace(async (dir) => {
    ...
    git(dir, "add", "b.ts");
    const reg = registry(dir, AGENT_A);
    await reg.acquire({ resourceSpecs: ["b.ts"], reason: "edit" });
...
test("F3 a path covered only by another agent lands in foreign_covered", async () => {
  await withWorkspace(async (dir) => {
    ...
    await writeFile(path.join(dir, "b.ts"), "hi\n");
    git(dir, "add", "b.ts");
    await registry(dir, AGENT_B).acquire({ resourceSpecs: ["b.ts"], reason: "B holds it" });
    const report = await verify(registry(dir, AGENT_A), dir);
    expect(report.uncovered).toEqual([]);
...
test("F3 a reclaimable (dead) lock does NOT cover a staged path", async () => {
  await withWorkspace(async (dir) => {
    ...
    await staleReg.acquire({ resourceSpecs: ["b.ts"], reason: "stale", ttlMs: 1000 });
